...
import { requestVerify } from "../request/api/login";
...
export default {
  ...
  methods: {
    ...
    async testLoginState() {
      ...
      if (token) {
        let tokenValidity = await requestVerify({ token });
        console.log("token检测结果", tokenValidity.data.isValid);
        if(!tokenValidity.data.isValid) {
